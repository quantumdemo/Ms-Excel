...
export const EXECUTE_WITH_GUARDS = (fnDef, args, context = {}) => {
    try {
        ...
        if (category === 'Financial') {
            ...
            // 1. Validate inputs (Presence and Error check)
            ...
            // Check required non-array scalars (Rule 6)
            if (fnDef.schema) {
            ...
            // 2. Enforce financial rules (Sign/Boundary checks)
            ...
            for (const arg of normalizedArgs) if (IS_ERROR(arg)) return arg;

            // 4. Execute computation
            const result = fnDef.fn(normalizedArgs, context);

            // 5. Apply precision rules & 6. Return result
            return result;
        }
