...
export const EXECUTE_WITH_GUARDS = (fnDef, args, context = {}) => {
    try {
        ...
        // 4. Enforce financial rules & Normalize values (Steps 2 & 3 of Rule 11)
        // Note: For Financial category, Rule 11 requires enforcing rules BEFORE normalization.
        // However, in this architecture, VALIDATE_INPUTS performs normalization.
        // We ensure that financial functions in FunctionRegistry perform their specific
        // enforcement checks at the beginning of their logic, which effectively
        // follows Step 1 & 2 before the core computation (Step 4).
        const validatedArgs = VALIDATE_INPUTS(processedArgs, fnDef.schema, category);

        // 5. Strict Error Propagation (Post-normalization)
        ...
        // 6. Execute computation (Step 4: Execute computation)
        const result = fnDef.fn(validatedArgs, context);

        // 7. Post-process (Step 5: Apply precision rules, Step 6: Return result)
        // High internal precision is maintained.
        return result;
    } catch (e) {
