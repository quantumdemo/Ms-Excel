...
export const UNIQUE = (array, byCol = false, exactlyOnce = false) => {
    ...
    let data = Array.isArray(array[0]) ? array : array.map(v => [v]);
    ...
    data.forEach(row => {
        ...
        if (counts.get(s) === 1) rows.push(row);
    });

    let result = rows;
    if (exactlyOnce) {
        result = rows.filter(row => counts.get(JSON.stringify(row)) === 1);
    }
    ...
    return (result.length > 0 && result[0].length === 1 && !Array.isArray(array[0])) ? result.flat() : result;
};
...
export const SORTBY = (array, ...args) => {
  ...
  const data = Array.isArray(array[0]) ? array : array.map(v => [v]);
  const sortKeys = [];
  ...
  for (let i = 0; i < args.length; i += 2) {
    ...
    if (!Array.isArray(byArray) || byArray.length !== rowCount) {
      return "#VALUE!";
    }
  ...
  const rows = data.map((row, index) => ({
    row,
    index, // preserve original order for stable sort
    keys: sortKeys.map(k => extractKey(k.byArray, index))
  }));

  rows.sort((a, b) => {
    for (let i = 0; i < sortKeys.length; i++) {
      const { order } = sortKeys[i];
      const valA = a.keys[i];
      const valB = b.keys[i];

      const cmp = excelCompare(valA, valB, order);
      if (cmp !== 0) return cmp;
    }
  ...
  // Maintain 2D structure for spill engine, but flatten if input was 1D
  return (result[0].length === 1 && !Array.isArray(array[0])) ? result.flat() : result;
};
