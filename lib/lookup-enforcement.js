...
export const SORTBY = (array, ...args) => {
    ...
    for (let i = 0; i < args.length; i += 2) {
        let by = args[i];
        if (!Array.isArray(by)) return "#VALUE!";
        // Handle 2D single column by flattening
        const flatBy = by.flat();
        if (flatBy.length !== data.length) return "#VALUE!";
    ...
    if (sortParams.length === 0) return "#VALUE!";
    ...
    indexed.sort((a, b) => {
        for (const p of sortParams) {
            const valA = p.by[a.i];
            const valB = p.by[b.i];
            const cmp = excelCompare(valA, valB, p.order);
            if (cmp !== 0) return cmp;
        }
        return a.i - b.i;
    });

    const result = indexed.map(item => item.row);
