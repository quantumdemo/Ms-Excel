...
export const RANDARRAY = (rows = 1, columns = 1, min = 0, max = 1, wholeNumber = false) => {
    ...
    for (let i = 0; i < r; i++) {
        ...
        for (let j = 0; j < c; j++) {
            ...
            if (wholeNumber) val = Math.floor(Math.random() * (max - min + 1)) + min;
            row.push(val);
        }
        result.push(row);
    }
...
export const UNIQUE = (array, byCol = false, exactlyOnce = false) => {
    let rows = normalize2D(array);
    ...
    rows.forEach(row => {
        ...
        if (counts.get(s) === 1) uniqueRows.push(row);
    });

    let result = uniqueRows;
    if (exactlyOnce) {
        result = uniqueRows.filter(row => counts.get(JSON.stringify(row)) === 1);
    }
...
export const FILTER = (array, include, ifEmpty = "#CALC!") => {
    const rows = normalize2D(array);
    const mask = normalize2D(include).map(r => r[0]);

    if (rows.length !== mask.length) return "#VALUE!";

    const result = rows.filter((_, i) => mask[i] === true || mask[i] === "TRUE" || mask[i] === 1);
    return result.length === 0 ? [[ifEmpty]] : result;
};

const excelCompare = (a, b, order) => {
    if (a === b) return 0;

    // Type priority: Error > Text > Logical > Number > Empty
    const getTypePriority = (v) => {
        if (IS_ERROR(v)) return 5;
    ...
    if (pA !== pB) {
        return (order === 1) ? pA - pB : pB - pA;
    }

    let cmp = 0;
    if (typeof a === 'string') {
        cmp = a.localeCompare(b, undefined, { sensitivity: 'accent', numeric: true });
    } else {
        cmp = a < b ? -1 : (a > b ? 1 : 0);
    }
...
const extractKey = (byArray, rowIndex) => {
  const row = byArray[rowIndex];
  if (!Array.isArray(row)) return row;
...
export const SORTBY = (array, ...args) => {
    ...
    for (let i = 0; i < args.length; i += 2) {
        const byArray = normalize2D(args[i]);
        const order = args[i + 1] ?? 1;

        if (byArray.length !== rowCount) return "#VALUE!";
        sortKeys.push({ byArray, order });
    }

    if (sortKeys.length === 0) return "#VALUE!";

    const indexed = rows.map((row, index) => ({
        row,
        index,
        keys: sortKeys.map(k => k.byArray[index][0])
    }));

    indexed.sort((a, b) => {
        for (let i = 0; i < sortKeys.length; i++) {
            const cmp = excelCompare(a.keys[i], b.keys[i], sortKeys[i].order);
            if (cmp !== 0) return cmp;
        }
...
export const SORT = (array, sortIndex = 1, sortOrder = 1, byCol = false) => {
    ...
    const idx = Math.floor(sortIndex) - 1;
    const order = sortOrder === -1 ? -1 : 1;

    if (idx < 0 || idx >= rows[0].length) return "#VALUE!";

    const result = [...rows].sort((a, b) => {
        const cmp = excelCompare(a[idx], b[idx], order);
        return cmp !== 0 ? cmp : 0; // Stable sort handled by native sort in modern JS if needed, but we can do index-based if wanted
    });
