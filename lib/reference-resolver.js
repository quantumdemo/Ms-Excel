/**
 * ReferenceResolver handles conversions between Excel-style cell identifiers (e.g., "A1", "$A$1", "Sheet2!A1", "A:A")
 * and 0-indexed coordinates { r, c }.
 */
export class ReferenceResolver {
  static parseSheetReference(refStr) {
    if (!refStr || typeof refStr !== 'string') return { sheetName: null, cleanRef: refStr };

    const exclIdx = refStr.lastIndexOf('!');
    if (exclIdx === -1) return { sheetName: null, cleanRef: refStr };

    let sheetName = refStr.substring(0, exclIdx).trim();
    if ((sheetName.startsWith("'") && sheetName.endsWith("'")) || (sheetName.startsWith('"') && sheetName.endsWith('"'))) {
      sheetName = sheetName.substring(1, sheetName.length - 1);
    }
    const cleanRef = refStr.substring(exclIdx + 1).trim();
    return { sheetName, cleanRef };
  }
  ...
  static parseReference(ref) {
    if (!ref) return null;
    const { cleanRef } = ReferenceResolver.parseSheetReference(ref);
    const match = cleanRef.toUpperCase().match(/(\$?)([A-Z]+)(\$?)([0-9]+)/);
    if (!match) return null;
  ...
  static isRange(expr) {
    ...
    const { cleanRef } = ReferenceResolver.parseSheetReference(expr);
    return /^(\$?[A-Z]+\$?[0-9]+):(\$?[A-Z]+\$?[0-9]+)$/i.test(cleanRef) || /^(\$?[A-Z]+):(\$?[A-Z]+)$/i.test(cleanRef);
  }

  static getRangeCoords(rangeStr, maxRows = 120) {
    if (!rangeStr) return null;
    const { cleanRef } = ReferenceResolver.parseSheetReference(rangeStr);
