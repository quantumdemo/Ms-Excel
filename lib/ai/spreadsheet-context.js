...
export function buildSpreadsheetContext(registry, selectedCoords = { r: 0, c: 0 }) {
  if (!registry) {
    return {
      ...
      sheetName: "Sheet1",
      dataBounds: { lastRowNumber: 1, totalPopulatedRows: 0 },
      selectedCell: null,
      headers: [],
      sampleData: [],
      nearbyCells: [],
  ...
  // Non-empty, formula, and error cells sampling
  const nonEmptyCells = [];
  const formulaCells = [];
  const errors = [];
  ...
  // Extract compact 2D sample data matrix
  const sampleData = [];
  for (let r = 0; r < Math.min(lastRowNumber, 15); r++) {
    const rowObj = {};
