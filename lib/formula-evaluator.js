...
export class FormulaEvaluator {
  ...
  evaluate(formula, context = { scope: {}, sourceCell: null, cycleContext: null }) {
    ...
    const fullContext = {
        ...
        scope: context.scope || {},
        cycleContext: context.cycleContext || { timestamp: Date.now() }
    };

    try {
      const result = this.parseAndEval(expression, fullContext);
      if (!Array.isArray(result)) return [[result]];
      if (result.length > 0 && !Array.isArray(result[0])) return result.map(v => [v]);
      return result;
    } catch (e) {
  ...
  parseAndEval(expr, context) {
    ...
    if (funcMatch) {
      ...
      if (funcName === 'SHEET' || funcName === 'SHEETS') {
          if (this.registry.workbook) {
            return this.registry.workbook.sheets.size;
          }
          return 1;
      }

      if (funcName === 'SUBTOTAL') {
          const code = Number(this.parseAndEval(rawArgs[0], context));
          if (isNaN(code)) return "#VALUE!";
          const ranges = rawArgs.slice(1).map(r => r.trim());
          const funcDef = this.findFunction('SUBTOTAL');
          return EXECUTE_WITH_GUARDS(funcDef, [code, ...ranges], context);
      }
    ...
    // 3. Cross-Sheet Cell Refs & Ranges (e.g. Sheet2!A1, Sheet2!B2:B10)
    if (expr.includes('!')) {
      const { sheetName, cleanRef } = ReferenceResolver.parseSheetReference(expr);
      if (sheetName && this.registry.workbook) {
        if (/^\$?[A-Z]+\$?[0-9]+$/i.test(cleanRef)) {
          const cell = this.registry.workbook.getCell(cleanRef, sheetName);
          return cell.computed === undefined ? null : cell.computed;
        }
        if (ReferenceResolver.isRange(cleanRef)) {
          return this.registry.workbook.getRangeValues(cleanRef, sheetName);
        }
      }
    }

    // 4. Same-Sheet Cell Refs / Ranges
  ...
  getOperatorDef(op) {
    ...
    const comparisonSchema = [{ type: 'any', optional: false }, { type: 'any', optional: false }];
    ...
    const ops = {
        ...
        '>': { category: 'Logical', schema: comparisonSchema, fn: ([a, b]) => (a || 0) > (b || 0) },
        '<': { category: 'Logical', schema: comparisonSchema, fn: ([a, b]) => (a || 0) < (b || 0) },
        '>=': { category: 'Logical', schema: comparisonSchema, fn: ([a, b]) => (a || 0) >= (b || 0) },
        '<=': { category: 'Logical', schema: comparisonSchema, fn: ([a, b]) => (a || 0) <= (b || 0) },
        '=': { category: 'Logical', schema: comparisonSchema, fn: ([a, b]) => a === b },
        '<>': { category: 'Logical', schema: comparisonSchema, fn: ([a, b]) => a !== b }
    };
  ...
  resolveToken(token, context) {
    ...
    if (token.startsWith('(') && token.endsWith(')')) {
        return this.parseAndEval(token.slice(1, -1), context);
    }

    if (token.includes('!')) {
      const { sheetName, cleanRef } = ReferenceResolver.parseSheetReference(token);
      if (sheetName && this.registry.workbook) {
        if (/^\$?[A-Z]+\$?[0-9]+$/i.test(cleanRef)) return this.registry.workbook.getCell(cleanRef, sheetName).computed;
        if (ReferenceResolver.isRange(cleanRef)) return this.registry.workbook.getRangeValues(cleanRef, sheetName);
      }
    }
