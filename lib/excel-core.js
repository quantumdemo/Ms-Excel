...
export class CellRegistry {
  ...
  updateCell(id, rawValue) {
    ...
    if (raw.startsWith("=")) {
      ...
      // Extract cell references, including absolute markers
      const refRegex = /\$?[A-Z]+\$?[0-9]+/g;
      const rawDeps = raw.toUpperCase().match(refRegex) || [];
      // Clean dependencies for registry tracking
      dependencies = Array.from(new Set(rawDeps.map(d => d.replace(/\$/g, ''))));
