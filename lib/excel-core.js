...
export class CellRegistry {
  ...
  computeValue(id, cycleContext = { timestamp: Date.now() }) {
    ...
    if (cell.type === "formula") {
      ...
      // Trigger dependents of cells that were previously spilled into or are newly spilled into
      const allAffectedSpillIds = Array.from(new Set([...previousSpills, ...(Array.isArray(cell.computed) ? cell.computed.flat().map((_, i) => {
          // This is a bit lazy, let's just trigger based on the spillIds we collected
          return [];
      }).flat() : [])]));

      // Better: just trigger recalculation for anything that was a spill and is no longer, or is now a spill
  ...
  loadData(data) {
    ...
    // Pass 1: Set raw values and formulas
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        // Extract dependencies for formulas
        if (type === "formula") {
          const formulaWithoutStrings = raw.replace(/"([^"\\]|\\.)*"/g, "");
          const rangeRegex = /\$?[A-Z]+\$?[0-9]+:\$?[A-Z]+\$?[0-9]+/g;
          const refRegex = /\$?[A-Z]+\$?[0-9]+/g;
    ...
    // Pass 2: Recalculate everything in topological order
