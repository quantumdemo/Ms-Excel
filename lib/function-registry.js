...
const iterateNewton = (fn, dfn, guess = 0.1, maxIter = 100, tolerance = 1e-9) => {
    let x = guess;
    const damping = 0.8;
    for (let i = 0; i < maxIter; i++) {
        ...
        // Safeguard against division by zero or extremely flat gradients
        if (Math.abs(dy) < 1e-12) {
            x += 0.01; // Nudge and continue
            continue;
        }

        const delta = y / dy;
        const nextX = x - damping * delta;

        // Convergence check
        if (Math.abs(nextX - x) < tolerance) return nextX;

        // Prevent jumping into invalid domains (e.g. rate < -1)
        if (nextX <= -0.999) {
            x = (x - 1) / 2;
        } else {
            x = nextX;
